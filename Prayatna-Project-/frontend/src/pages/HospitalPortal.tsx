...
function RequestRow({ type, name, phone, detail, status, time, onAccept, onReject }: {
  ...
  return (
    <div className="bg-[#1a6040] border border-teal-800/30 rounded-xl p-4 flex items-center gap-4 transition-all hover:border-teal-600/40">
...
function AmbulanceAlertRow({ notif, onAccept, onReject }: { notif: AmbulanceNotif; onAccept: () => void; onReject: () => void }) {
  ...
  return (
    <div className={`border rounded-xl p-4 transition-all ${isPending ? 'bg-gradient-to-r from-rose-950/30 to-[#1a6040] border-rose-800/40 animate-pulse-slow' : 'bg-[#1a6040] border-teal-800/30'}`}>
...
export default function HospitalPortal() {
  ...
  if (profileError) {
    return (
      <div className="flex h-screen bg-[#134832] items-center justify-center">
        <div className="text-center p-8 bg-[#1c6845] border border-rose-700/40 rounded-2xl max-w-md">
  ...
  return (
    <div className="flex h-screen bg-[#134832] font-sans overflow-hidden">
      {/* Left Sidebar */}
      <div className="w-64 bg-[#175438] border-r border-teal-900/40 flex flex-col shrink-0">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Top Header */}
        <header className="bg-[#1b6242] border-b border-teal-900/40 px-6 py-4 flex items-center justify-between shrink-0">
          <div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {[...emergencies, ...appointments.map(a => ({ ...a, title: a.reason, description: a.appointmentType, patientName: a.patientName, priority: 'low' }))]
                    ...
                    .slice(0, 5)
                    .map((item: any) => (
                      <div key={item.id} className="bg-[#1a6040] border border-teal-800/30 rounded-lg p-3 flex items-center gap-3">
                    ...
                    {editingInventory && (
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        <input
                          ...
                          className="flex-1 bg-[#114030] border border-teal-800/40 rounded-lg px-3 py-1.5 text-white text-center text-sm font-bold focus:border-teal-500 focus:outline-none"
                        />
                ...
                <div className="bg-gradient-to-br from-red-900/40 to-red-950/30 border-red-700/40 border rounded-2xl p-5 transition-all hover:scale-[1.01]">
                  ...
                  {editingInventory && (
                    <input
                      ...
                      className="w-full bg-[#114030] border border-teal-800/40 rounded-lg px-3 py-1.5 text-white text-sm font-bold focus:border-teal-500 focus:outline-none mt-2"
            ...
            <div className="space-y-4">
              ...
              {sharingView === 'browse' && (
                <div className="space-y-3">
                  ...
                  {allHospitals.map(h => (
                    <div key={h.id} className="bg-[#1a6040] border border-teal-800/30 rounded-xl p-4 hover:border-teal-600/40 transition-all">
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {[
                          ...
                          <div key={stat.label} className="bg-[#175438] rounded-lg p-2 border border-teal-900/30 text-center">
                ...
                <div className="space-y-3">
                  {incomingRequests.map(req => (
                    <div key={req.id} className={`border rounded-xl p-4 transition-all ${
                      req.status === 'pending' ? 'bg-gradient-to-r from-amber-950/20 to-[#1a6040] border-amber-700/40' :
                      req.status === 'agreed' ? 'bg-[#1a6040] border-emerald-800/30' : 'bg-[#1a6040] border-rose-900/30'
                    }`}>
              ...
              {sharingView === 'outgoing' && (
                <div className="space-y-3">
                  {outgoingRequests.map(req => (
                    <div key={req.id} className={`border rounded-xl p-4 transition-all ${
                      req.status === 'pending' ? 'bg-[#1a6040] border-teal-800/30' :
                      req.status === 'agreed' ? 'bg-gradient-to-r from-emerald-950/20 to-[#1a6040] border-emerald-800/40' :
                      'bg-gradient-to-r from-rose-950/10 to-[#1a6040] border-rose-900/30'
                    }`}>
                      <div className="flex items-center gap-2 mb-1">
              ...
              {requestModal.open && requestModal.hospital && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#175438]/90 backdrop-blur-sm p-4">
                  <div className="bg-[#1d6a46] border border-teal-700/50 rounded-2xl w-full max-w-md shadow-2xl">
                    ...
                    <div className="p-5 space-y-4">
                      ...
                      <div>
                        ...
                        <select value={requestForm.resourceType} onChange={e => setRequestForm(p => ({ ...p, resourceType: e.target.value }))}
                          className="w-full bg-[#175438] border border-teal-800/50 rounded-lg px-4 py-2.5 text-white focus:border-teal-500 outline-none">
                      ...
                      <div>
                        ...
                        <textarea value={requestForm.message} onChange={e => setRequestForm(p => ({ ...p, message: e.target.value }))} rows={3} placeholder="e.g. Critical patient needs oxygen immediately"
                          className="w-full bg-[#175438] border border-teal-800/50 rounded-lg px-4 py-2.5 text-white focus:border-teal-500 outline-none resize-none placeholder:text-teal-700" />
                      </div>
      ...
      {profilePanelOpen && (
        <div className="fixed inset-0 z-[200] flex justify-end">
          ...
          {/* Panel */}
          <div className="relative w-full max-w-lg bg-[#175438] border-l border-teal-800/50 h-full overflow-y-auto shadow-2xl flex flex-col">
            {/* Panel Header */}
            <div className="sticky top-0 z-10 bg-[#1a6040]/95 backdrop-blur-sm border-b border-teal-800/40 px-6 py-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
            ...
            <div className="flex-1 p-6 space-y-6">
              ...
              {/* CSV Upload Section */}
              <div className="bg-gradient-to-br from-teal-900/40 to-[#134832] border border-teal-800/40 rounded-xl p-4">
              ...
              <div className="space-y-3">
                ...
                {([
                  ...
                  <div key={key} className="flex items-center gap-3 bg-[#1a6040] border border-teal-800/30 rounded-xl px-4 py-3">
                ...
                {editingProfile && (
                  <div className="grid grid-cols-2 gap-2">
                    {([{ label: 'Latitude', key: 'latitude' }, { label: 'Longitude', key: 'longitude' }] as const).map(({ label, key }) => (
                      <div key={key} className="bg-[#1a6040] border border-teal-800/30 rounded-xl px-4 py-3">
                        <p className="text-teal-500/50 text-[9px] uppercase tracking-wider">{label}</p>
                        <input value={(profileForm as any)[key]} onChange={e => setProfileForm(p => ({ ...p, [key]: e.target.value }))}
                          className="w-full bg-transparent text-white text-sm outline-none border-b border-teal-700/40 focus:border-teal-400 mt-0.5 py-0.5" />
                      </div>
              ...
              <div className="space-y-3">
                ...
                <div className="bg-[#1a6040] border border-teal-800/30 rounded-xl px-4 py-3">
